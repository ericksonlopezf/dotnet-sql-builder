...
| `AotQueryAsync<T>` | User-provided `Func<IDataReader, T>` | ✅ Always |
...
|-----------|-------|-----------|
| Dapper `QueryAsync<T>` | Reflection-based object mapper | Use `AotQueryAsync<T>` |
...
3. **`AotQueryAsync<T>`** — for query execution without Dapper reflection
...
- Verification that `AotQueryAsync<T>` executes in NativeAOT
...
### Negative
- Standard Dapper `QueryAsync<T>` path is not AOT — users must use `AotQueryAsync<T>` for NativeAOT scenarios

## API Impact
Documentation must clearly label:
- `QueryAsync<T>` — NOT NativeAOT compatible
- `AotQueryAsync<T>` — NativeAOT compatible (requires user mapper function)
- `BulkBuilder<T>` with SrcGen — NativeAOT compatible

## Reconsideration Criteria
When Dapper.AOT achieves full NativeAOT compatibility, evaluate whether to route through it instead of our custom `AotQueryAsync<T>` path.
