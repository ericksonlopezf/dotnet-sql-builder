...
### Missing Benchmarks (Gaps)

| Benchmark | Priority | Reason |
|-----------|---------|--------|
| AOT path (`AotSqlRendererBase.RenderInsert<T>`) | P1 | Core AOT claim must be measurable |
| AOT path vs. Dapper `QueryAsync<T>` hydration | P1 | Demonstrates `QueryAotAsync<T>` value |
| Bulk insert (`BulkBuilder<T>`) | P1 | Validates bulk strategy performance claims |
| Window function compilation | P2 | Complex node traversal |
| Composite cursor (`SeekAfter` 3-key) | P2 | High-value advanced feature |
| CTE compilation (5+ CTEs) | P2 | Nested structure cost |
| Multi-JOIN query (5 JOINs) | P2 | Join partition overhead |
| `ApplyDiff` (diff UPDATE) | P2 | ISqlEntity traversal cost |
| `Expression.Compile()` first-call cost | P1 | NativeAOT warning gate |
| `Expression.Compile()` cached-call cost | P1 | Shows amortized cost |
| `SqlEntityCache<T>` static init cost | P2 | Per-type one-time cost |
| Concurrent query build (thread safety) | P2 | Immutability proof |
| 10k parameter query (parameter manager) | P3 | Stress / edge case |
| PostgreSQL compiler vs. SQL Server | P3 | Dialect overhead |
