...
> Last audit: 2026-08-14 | Based on direct compiler source inspection.
...
| Feature | SS | PG | MY | LT | OR | Class |
|---------|----|----|----|----|-----|-------|
| Identifier quoting | `[x]` | `"x"` | `` `x` `` | `"x"` | `"X"` UPPER | 🔵 DialectSpecific |
| Case sensitivity | Insensitive | Case-preserved in `""` | Insensitive | Insensitive | UPPERCASE unless quoted | 🔵 |
...
|---------|----|----|----|----|-----|-------|
| Parameter prefix | `@` | `@` | `@` | `@` | `:` | 🔵 |
| Max parameters | 2100 | None | None | None | None | 🔵 |
...
| `BETWEEN a AND b` | ✅ | ✅ | ✅ | ✅ | ✅ | 🌐 |
| `LIKE '%x%'` | ✅ | ✅ | ✅ | ✅ | ✅ | 🌐 |
| `ILIKE` (case-insensitive LIKE) | ❌ | ✅ | 🟡→LIKE | ❌ | ❌ | 🔵 (not exposed in API) |
...
| Feature | SS | PG | MY | LT | OR | Class |
|---------|----|----|----|----|-----|-------|
| Unified `.Returning()` API | 🟡→OUTPUT | ✅ | ❌ throws | ✅ | 🟡→INTO | 🟡 |
| From INSERT | ✅ | ✅ | ❌ | ✅ | ✅ | 🔵 |
| From UPDATE | ✅ | ✅ | ❌ | ✅ | ✅ | 🔵 |
| From DELETE | ✅ | ✅ | ❌ | ✅ | ✅ | 🔵 |
...
| Strategy | SS | PG | MY | LT | OR | Class | Safety |
|---------|----|----|----|----|-----|-------|--------|
...
| Named WINDOW clause | ✅ | ✅ | ✅ | ✅ | ✅ | 🌐 |
| FRAME (ROWS / RANGE) | ❌ API | ❌ API | ❌ API | ❌ API | ❌ API | 🔵 (planned — raw only) |
| FILTER (WHERE) in window | 📋 v1.3 | 📋 v1.3 | 📋 v1.3 | 📋 v1.3 | 📋 v1.3 | 🔵 ADR-018 |
...
| Type | SS | PG | MY | LT | OR | Notes |
|------|----|----|----|----|-----|-------|
| `bool` | `BIT` | `BOOLEAN` | `TINYINT(1)` | `INTEGER` | `NUMBER(1)` | Driver handles |
| `Guid` / `UUID` | `UNIQUEIDENTIFIER` | `UUID` | `CHAR(36)` | `TEXT` | `RAW(16)` | `RegisterTypeHandler<T>` needed |
| `DateTimeOffset` | ✅ | ✅ | ⚠️ | ❌ | ✅ | MY stores as UTC only |
| JSON column | `NVARCHAR`/`JSON` | `jsonb`/`json` | `JSON` | `TEXT` | `CLOB` | No typed API in ESQL |
