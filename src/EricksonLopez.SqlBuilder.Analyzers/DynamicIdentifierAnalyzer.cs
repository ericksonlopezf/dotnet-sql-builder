    ...
    public class DynamicIdentifierAnalyzer : DiagnosticAnalyzer
    {
        ...
        private static bool IsDynamicIdentifier(ExpressionSyntax expression, SyntaxNodeAnalysisContext context)
        {
                ...
                binary.IsKind(SyntaxKind.AddExpression))
            {
                ...
                var leftConst = context.SemanticModel.GetConstantValue(binary.Left);
                var rightConst = context.SemanticModel.GetConstantValue(binary.Right);
            ...
            if (expression is InterpolatedStringExpressionSyntax interpolated)
            {
                foreach (var content in interpolated.Contents)
                {
                    ...
                    {
                        var constValue = context.SemanticModel.GetConstantValue(interpolation.Expression);
