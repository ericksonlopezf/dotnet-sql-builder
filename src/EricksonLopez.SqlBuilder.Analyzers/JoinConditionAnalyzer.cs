                ...
                if (methodName == "Join" || methodName == "LeftJoin" || methodName == "RightJoin" || methodName == "InnerJoin")
                {
                    if (invocation.ArgumentList.Arguments.Count > 0)
                    {
                        ...
                        if (arg is ParenthesizedLambdaExpressionSyntax lambda && lambda.Body is BinaryExpressionSyntax binary)
                        {
                            ...
                            {
                                var leftSymbol = context.SemanticModel.GetSymbolInfo(binary.Left).Symbol as IPropertySymbol;
                                var rightSymbol = context.SemanticModel.GetSymbolInfo(binary.Right).Symbol as IPropertySymbol;
