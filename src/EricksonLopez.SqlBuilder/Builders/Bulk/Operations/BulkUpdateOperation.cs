...
internal sealed class BulkUpdateOperation<T> : IBulkOperation<T> where T : IStaticEntityMetadata<T>
{
    ...
    public BulkUpdateOperation(IEnumerable<T> entities, List<IColumnSelectionRule<T>> rules, int batchSize)
    {
        _entities = entities;
        _rules = rules;
    ...
    {
        return dialect.RenderBulkUpdate(_entities, _rules, _batchSize);
    }
}
