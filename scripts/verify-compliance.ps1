...
<#
.SYNOPSIS
    Architecture & Quality Standards Compliance Verification Script for EricksonLopez.SqlBuilder.
.DESCRIPTION
    ...
    5. Valid GitHub repository links referencing ericksonlopezf/dotnet-sql-builder.
...
Write-Host "  Repository: EricksonLopez.SqlBuilder            " -ForegroundColor Cyan
Write-Host "==================================================" -ForegroundColor Cyan
...
# 1b. Documentation Link & Heading Hierarchy Integrity
Write-Host "`n[1b] Checking documentation link integrity and heading hierarchy..." -ForegroundColor Yellow
$allMdFiles = Get-ChildItem -Path $RootDirectory -Recurse -Filter "*.md" -ErrorAction SilentlyContinue | Where-Object { $_.FullName -notmatch '\\(bin|obj|StrykerOutput|TestResults|artifacts)\\' }
$brokenLinks = 0
$badH1Count = 0
foreach ($f in $allMdFiles) {
    ...
    foreach ($m in $linkMatches) {
        ...
        if ($link -match '^(http|https|mailto|ftp):' -or $link.StartsWith('#')) { continue }
        ...
        try {
            ...
            if (-not (Test-Path $target)) {
                Write-Host "  ❌ Broken link in $($f.FullName): $link" -ForegroundColor Red
                $violations++
                $brokenLinks++
            }
        } catch {
    ...
    if ($h1Count -gt 1) {
        Write-Host "  ❌ Multiple H1 headings ($h1Count) in $($f.FullName)" -ForegroundColor Red
        $violations++
        $badH1Count++
    }
}
if ($brokenLinks -eq 0) { Write-Host "  ✅ 100% relative Markdown links resolve correctly." -ForegroundColor Green }
if ($badH1Count -eq 0) { Write-Host "  ✅ All Markdown files enforce single H1 heading hierarchy." -ForegroundColor Green }

# 2. Zero Obsolete APIs in src/ (Zero tolerance policy)
Write-Host "`n[2/7] Checking for unmanaged [Obsolete] attribute usages in src/..." -ForegroundColor Yellow
$srcCsFiles = Get-ChildItem -Path (Join-Path $RootDirectory "src") -Recurse -Filter "*.cs" | Where-Object { $_.FullName -notmatch "\\(obj|bin)\\" }
$allowedObsoleteFiles = @() # Zero tolerance: No Obsolete APIs in the solution
$obsoleteCount = 0
foreach ($cs in $srcCsFiles) {
    if ($allowedObsoleteFiles.Count -gt 0 -and $allowedObsoleteFiles -contains $cs.Name) { continue }
...
if ($obsoleteCount -eq 0) { Write-Host "  ✅ Zero unmanaged [Obsolete] attributes in production code." -ForegroundColor Green }
...
Write-Host "`n[5/7] Checking GitHub identity links (ericksonlopezf/dotnet-sql-builder)..." -ForegroundColor Yellow
$badLinks = 0
...
foreach ($f in $allTrackedFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "github\.com/ericksonlopez/dotnet-sql-builder\b") {
...
if ($badLinks -eq 0) { Write-Host "  ✅ All GitHub URLs correctly target ericksonlopezf/dotnet-sql-builder." -ForegroundColor Green }
