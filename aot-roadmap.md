...
| `SqlEntityCache<T>` (without `[SqlEntity]`) | ✅ Guarded with `[RequiresUnreferencedCode]` / `[RequiresDynamicCode]` | None |
| `SqlExpressionVisitor` | ✅ Fully AOT & Visitor cached | None |
...
| `AotQueryAsync<T>()` (inferred mapper & user mapper) | ✅ Fully AOT | None |
| Dapper `QueryAsync<T>` | ⚠️ Reflection mapper (Dapper native limitation) | Expected |
| Source Generator output (`IStaticEntityMetadata<T>`) | ✅ Compile-time & AOT | None |
| Roslyn Analyzers (`ESQL001–026`) | ✅ Compile-time | None |
| `UnitOfWork` | ✅ Fully AOT | None |
| `SqlResilienceExtensions` | ✅ Fully AOT | None |
| `SqlBuilderInstrumentation` (OTel) | ✅ Fully AOT | None |
| `IsAotCompatible = true` in packages | ✅ Declared in all packages | None |
| CI NativeAOT Publish Gate | ✅ Enforced in `.github/workflows/ci.yml` | None |
