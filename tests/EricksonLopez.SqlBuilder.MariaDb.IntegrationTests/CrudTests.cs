...
public class CrudTests
{
    ...
    public CrudTests(MariaDbFixture fixture)
    {
        _fixture = fixture;
        EricksonLopez.SqlBuilder.Dapper.DapperExtensions.RegisterCompiler<MySqlConnector.MySqlConnection>(() => new MariaDbCompiler());
        global::Dapper.DefaultTypeMap.MatchNamesWithUnderscores = true;
    }

    // ─── Native RETURNING (key MariaDB integration test) ─────────────────────

    [Fact]
    public async Task Can_Insert_And_ReturnId_UsingNativeReturning()
    {
        // MariaDB 10.5+ supports RETURNING natively — this verifies end-to-end
        using var connection = _fixture.CreateConnection();
        var insertQuery = Sql.Insert<CrudUser>(new CrudUser
            {
                Name = "Jane Doe (RETURNING)",
                Age = 25,
                IsActive = true
            })
            .Returning(u => u.Id);
        ...
        user!.Name.Should().Be("Jane Doe (RETURNING)");
    }
