...
public class CrudTests
{
    ...
    public CrudTests(OracleFixture fixture)
    {
        _fixture = fixture;
        EricksonLopez.SqlBuilder.Dapper.DapperExtensions.RegisterCompiler<global::Oracle.ManagedDataAccess.Client.OracleConnection>(() => new OracleCompiler());
    ...
    public async Task Can_Insert_And_Select_User()
    {
        ...
        var insertQuery = Sql.Insert<OracleCrudUser>(new OracleCrudUser 
            {
                Name = "John Doe",
                Age = 30,
                IsActive = true
        ...
        // Act - Select
        var selectQuery = Sql.From<OracleCrudUser>()
            .Select("*")
            .Where(u => u.Id == insertedId);
        
        var user = (await connection.QueryAsync<OracleCrudUser>(selectQuery)).SingleOrDefault();
    ...
    public async Task Can_Update_User()
    {
        // Arrange
        using var connection = _fixture.CreateConnection();
        var insertQuery = Sql.Insert<OracleCrudUser>(new OracleCrudUser { Name = "Update Me", Age = 20, IsActive = true })
            .Returning(u => u.Id);
        
        var insertedId = (await connection.QueryAsync<int>(insertQuery)).Single();

        // Act - Update
        var updateQuery = Sql.Update<OracleCrudUser>()
        ...
        var selectQuery = Sql.From<OracleCrudUser>().Select("*").Where(u => u.Id == insertedId);
        var user = (await connection.QueryAsync<OracleCrudUser>(selectQuery)).Single();
    ...
    public async Task Can_Delete_User()
    {
        // Arrange
        using var connection = _fixture.CreateConnection();
        var insertQuery = Sql.Insert<OracleCrudUser>(new OracleCrudUser { Name = "Delete Me", Age = 99, IsActive = false })
            .Returning(u => u.Id);
        
        var insertedId = (await connection.QueryAsync<int>(insertQuery)).Single();

        // Act - Delete
        var deleteQuery = Sql.Delete<OracleCrudUser>()
            .Where(u => u.Id == insertedId);
    ...
    public async Task Can_BulkInsert_Users()
    {
        ...
        var users = new[]
        {
            new OracleCrudUser { Name = name1, Age = 20, IsActive = true },
            new OracleCrudUser { Name = name2, Age = 25, IsActive = false }
        };
        ...
        var selectQuery = Sql.From<OracleCrudUser>().Where(u => names.Contains(u.Name));
        var insertedUsers = (await connection.QueryAsync<OracleCrudUser>(selectQuery)).ToList();
    ...
    public async Task Can_BulkUpdate_Users()
    {
        ...
        {
            new OracleCrudUser { Name = name1, Age = 20, IsActive = true },
            new OracleCrudUser { Name = name2, Age = 25, IsActive = true }
        };

        await connection.ExecuteAsync(Sql.BulkInsert(users));
        var names = new List<string> { name1, name2 };
        var insertedUsers = (await connection.QueryAsync<OracleCrudUser>(Sql.From<OracleCrudUser>().Where(u => names.Contains(u.Name)))).ToList();
        var insertedIds = insertedUsers.Select(u => u.Id).ToList();

        // Act - Bulk Update
        var bulkUpdate = Sql.Update<OracleCrudUser>()
        ...
        var updatedUsers = (await connection.QueryAsync<OracleCrudUser>(Sql.From<OracleCrudUser>().Where(u => insertedIds.Contains(u.Id)))).ToList();
    ...
    public async Task Can_BulkDelete_Users()
    {
        ...
        // Act - Bulk Delete
        var bulkDelete = Sql.Delete<OracleCrudUser>()
            .Where(u => insertedIds.Contains(u.Id));
        ...
        // Assert
        rowsAffected.Should().Be(2);
        var remainingUsers = (await connection.QueryAsync<OracleCrudUser>(Sql.From<OracleCrudUser>().Where(u => insertedIds.Contains(u.Id)))).ToList();
